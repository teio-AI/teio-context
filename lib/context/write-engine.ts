...
export type { Identity, RepoRef } from '../github/git-data'
 ...
 * mock instead of hitting GitHub. Low-level Git Data + Merges calls live in
 * lib/github/git-data.ts, shared with the discover/import seeder.
 *
...
export class WriteEngine {
  ...
  private async tryFastPath(
    ...
    const blobSha = opts.baseBlob ?? (await getBlobSha(gh, repo, change.path, baseRef)) ?? undefined
  ...
  private async threeWay(gh: GitHubApi, repo: RepoRef, change: WriteChange, opts: WriteOptions): Promise<EngineResult> {
    const baseSha = opts.baseVersion ?? (await getBranchHead(gh, repo))
    const baseTree = await getCommitTree(gh, repo, baseSha)
    const entries = await this.buildTreeEntries(gh, repo, change, baseSha)
    const treeSha = await createTree(gh, repo, baseTree, entries)
    const headSha = await createCommit(gh, repo, {
      message: opts.message ?? defaultMessage(change),
      tree: treeSha,
      parents: [baseSha],
      author: opts.author,
    ...
    for (let attempt = 0; attempt < MERGE_RETRY_MAX; attempt++) {
      const merge = await mergeBranch(gh, repo, headSha, opts.message ?? defaultMessage(change))
      if (merge.status === 204) {
        ...
        return { status: 'merged', version: await getBranchHead(gh, repo), viaFastPath: false }
      }
...
export function validateContent(content: string): void {
  // Binary heuristic: a NUL byte never appears in legitimate UTF-8 markdown.
